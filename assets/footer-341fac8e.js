import{a as n,i as I}from"./vendor-db25513e.js";import{i as x}from"./modal_window-83bbecc9.js";const N=document.querySelector(".js-menu"),U=document.querySelector(".js-open-menu"),V=document.querySelector(".js-close-menu"),H=function(){const e=N.classList.toggle("is-open");document.body.style.overflow=e?"hidden":""};U.addEventListener("click",H);V.addEventListener("click",H);window.matchMedia("(min-width: 335px)").addEventListener("change",j);function j(e){e.matches&&(N.classList.remove("is-open"),document.body.style.overflow="")}let J=document.querySelector(".QuoteText"),z=document.querySelector(".QuoteAuthor");window.addEventListener("DOMContentLoaded",async function(){try{let e=localStorage.getItem("quoteData"),t=localStorage.getItem("quoteDate"),r=new Date,s=t?new Date(t):null;if(e&&s&&_(r,s)){k(JSON.parse(e));return}let a=(await n.get("https://energyflow.b.goit.study/api/quote")).data;localStorage.setItem("quoteData",JSON.stringify(a)),localStorage.setItem("quoteDate",r.toISOString()),k(a)}catch(e){console.error("Error:",e)}});function k(e){J.innerText=e.quote,z.innerText=e.author}function _(e,t){return e.getFullYear()===t.getFullYear()&&e.getMonth()===t.getMonth()&&e.getDate()===t.getDate()}const Y=document.querySelector(".FilterButtons"),i=document.querySelector(".ExerciseFiltersList"),h=document.querySelector(".Pagination"),Q=document.querySelector(".ExercisesForm"),K=document.querySelector(".ExerciseFiltersListSubcategories"),G=document.querySelector(".PaginationSubcategories"),X="https://energyflow.b.goit.study/api";let A="Muscles",v=1,B=window.innerWidth,y=0;Y.addEventListener("click",ee);h.addEventListener("click",O);B<=375?y=8:(B<=768,y=12);async function q(){try{return(await n.get(`${X}/filters`,{params:{filter:A,page:v,limit:y}})).data}catch(e){console.log(e)}}async function Z(){try{const e=await q().then(t=>{const{results:r,totalPages:s,page:o}=t;if(r&&r.length>0){i.innerHTML=L(r);const a=M(o,s);h.innerHTML=a}else console.error("No exercises found.")})}catch(e){console.error("Error fetching exercises:",e)}}Z();async function ee(e){if(e.target.tagName!=="BUTTON")return;e.preventDefault(),Q.classList.add("visually-hidden"),K.classList.add("visually-hidden"),i.classList.remove("visually-hidden"),h.innerHTML="",G.innerHTML="",v=1;const r=e.target.dataset.filter;if(A=r,i.innerHTML="",Array.from(e.currentTarget.children).map(s=>{s.textContent!==e.target.textContent?s.classList.remove("ButtonIsActive"):e.target.classList.add("ButtonIsActive")}),e.target.tagName==="BUTTON")try{q(r).then(s=>{const{results:o,totalPages:a,page:l}=s;i.innerHTML=L(o);{const u=M(l,a);h.innerHTML=u}})}catch(s){console.log(s)}}async function O(e){if(e.target.tagName==="BUTTON"){v=e.target.textContent,Array.from(e.currentTarget.children).map(t=>{t.textContent!==v?t.classList.remove("PaginationBtnIsActive"):e.target.classList.add("PaginationBtnIsActive")}),i.innerHTML="";try{const{results:t,page:r,totalPages:s}=await q();if(r===s)return;i.innerHTML=L(t)}catch(t){console.log(t)}}}function L(e){return e.map(({name:r,filter:s,imgUrl:o})=>` <li class='FilterList ExercisesItem' data-filter='${s}' data-name='${r}'>
        <img class="ImgExercises" src="${o}" alt="${s}">
        <div class="FilterText">
          <p class="FilterExercises">${r}</p>
          <p class="FilterName">${s}</p>
        </div>
      </li>`).join("")}function M(e,t){let r="";for(let s=1;s<=t;s+=1)r+=`<button class="PaginationBtn PaginationBtnIsActive" type="button">${s}</button>`;return r}const $=document.querySelector(".ExerciseFiltersList");document.querySelector(".ExercisesHead");const m=document.querySelector(".Pagination"),P=document.querySelector(".PaginationSubcategories"),w=document.querySelector(".ExerciseFiltersListSubcategories"),te=document.querySelector(".ExercisesForm"),f="https://energyflow.b.goit.study/api";let p=1,S,F;$.addEventListener("click",re);async function re(e){if(te.classList.remove("visually-hidden"),m.innerHTML="",$.classList.add("visually-hidden"),w.classList.remove("visually-hidden"),m.removeEventListener("click",O),m.removeEventListener("click",ae),e.target===e.currentTarget)return;const t=e.target.closest(".ExercisesItem");S=t.dataset.filter,F=t.dataset.name;try{const{page:r,totalPages:s,results:o}=await R(S,F);if(w.innerHTML=W(o),m.innerHTML="",s>1){const a=M(r,s);P.innerHTML=a}P.addEventListener("click",oe)}catch{g("Error")}}async function R(e,t,r){try{return e==="Muscles"?(await n.get(`${f}/exercises`,{params:{muscles:t,page:r,limit:9}})).data:e==="Body parts"?(await n.get(`${f}/exercises`,{params:{bodypart:t,page:r,limit:9}})).data:(await n.get(`${f}/exercises`,{params:{equipment:t,page:r,limit:9}})).data}catch{g("Error")}}function W(e){return e.map(({rating:r,name:s,burnedCalories:o,time:a,bodyPart:l,target:u,_id:T})=>`<li class="WorkoutCard ExerciseCategoryList id ='${T}">

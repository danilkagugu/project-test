import{a as d,i as w}from"./vendor-db25513e.js";import{i as h}from"./modal_window-48f78ea4.js";let W=document.querySelector(".QuoteText"),U=document.querySelector(".QuoteAuthor");window.addEventListener("DOMContentLoaded",async function(){try{let e=localStorage.getItem("quoteData"),t=localStorage.getItem("quoteDate"),r=new Date,s=t?new Date(t):null;if(e&&s&&J(r,s)){I(JSON.parse(e));return}let n=(await d.get("https://energyflow.b.goit.study/api/quote")).data;localStorage.setItem("quoteData",JSON.stringify(n)),localStorage.setItem("quoteDate",r.toISOString()),I(n)}catch(e){console.error("Error:",e)}});function I(e){W.innerText=e.quote,U.innerText=e.author}function J(e,t){return e.getFullYear()===t.getFullYear()&&e.getMonth()===t.getMonth()&&e.getDate()===t.getDate()}const R=document.querySelector(".FilterButtons"),f=document.querySelector(".ExerciseFiltersList"),F=document.querySelector(".Pagination"),V="https://energyflow.b.goit.study/api";let C="Muscles",L=1,M=window.innerWidth,E=0;R.addEventListener("click",j);F.addEventListener("click",b);M<=375?E=8:(M<=768,E=12);async function T(){try{return(await d.get(`${V}/filters`,{params:{filter:C,page:L,limit:E}})).data}catch(e){console.log(e)}}async function z(){try{const e=await T().then(t=>{const{results:r,totalPages:s,page:a}=t;if(r&&r.length>0){f.innerHTML=y(r);const n=B(a,s);F.innerHTML=n}else console.error("No exercises found.")})}catch(e){console.error("Error fetching exercises:",e)}}z();async function j(e){e.preventDefault(),L=1;const r=e.target.dataset.filter;if(C=r,f.innerHTML="",Array.from(e.currentTarget.children).map(s=>{s.textContent!==e.target.textContent?s.classList.remove("ButtonIsActive"):e.target.classList.add("ButtonIsActive")}),e.target.tagName==="BUTTON")try{T(r).then(s=>{const{results:a,totalPages:n,page:l}=s;f.innerHTML=y(a);{const u=B(l,n);F.innerHTML=u}})}catch(s){console.log(s)}}async function b(e){L=e.target.textContent,Array.from(e.currentTarget.children).map(t=>{t.textContent!==L?t.classList.remove("PaginationBtnIsActive"):e.target.classList.add("PaginationBtnIsActive")}),f.innerHTML="";try{const{results:t,page:r,totalPages:s}=await T();if(r===s)return;f.innerHTML=y(t)}catch(t){console.log(t)}}function y(e){return e.map(({name:r,filter:s,imgUrl:a})=>` <li class='FilterList ExercisesItem' data-filter='${s}' data-name='${r}'>
        <img class="ImgExercises" src="${a}" alt="${s}">
        <div class="FilterText">
          <p class="FilterExercises">${r}</p>
          <p class="FilterName">${s}</p>
        </div>
      </li>`).join("")}function B(e,t){let r="";for(let s=1;s<=t;s+=1)r+=`<button class="PaginationBtn PaginationBtnIsActive" type="button">${s}</button>`;return r}const i=document.querySelector(".ExerciseFiltersList"),_=document.querySelector(".ExercisesHead"),o=document.querySelector(".Pagination"),x="https://energyflow.b.goit.study/api";let g=1,c,v;i.addEventListener("click",q);async function q(e){if(i.removeEventListener("click",q),o.removeEventListener("click",b),o.removeEventListener("click",N),e.target===e.currentTarget)return;i.classList.add("ExerciseCategoryList"),i.classList.remove("ExerciseFiltersList");const t=e.target.closest(".ExercisesItem");c=t.dataset.filter,v=t.dataset.name;try{_.innerHTML=Q(v);const r=document.querySelector("#MusclesBtn");c==="Muscles"?r.classList.add("FilterBtnIsActive"):c==="Body parts"?document.querySelector("#BodyPartBtn").classList.add("FilterBtnIsActive"):c==="Equipment"&&document.querySelector("#EquipmentBtn").classList.add("FilterBtnIsActive");const{page:s,totalPages:a,results:n}=await A(c,v);i.innerHTML=D(n);const l=document.querySelector("#FilterBtn");if(l.addEventListener("click",Y),l.addEventListener("click",$),o.innerHTML="",a>1){const u=B(s,a);o.innerHTML=u}o.addEventListener("click",P)}catch{m("Error")}}function $(){document.querySelector(".ExercisesForm").remove(),FilterBtn.removeEventListener("click",$)}async function A(e,t,r){try{return e==="Muscles"?(await d.get(`${x}/exercises`,{params:{muscles:t,page:r,limit:9}})).data:e==="Body parts"?(await d.get(`${x}/exercises`,{params:{bodypart:t,page:r,limit:9}})).data:(await d.get(`${x}/exercises`,{params:{equipment:t,page:r,limit:9}})).data}catch{m("Error")}}function D(e){return e.map(({rating:r,name:s,burnedCalories:a,time:n,bodyPart:l,target:u,_id:O})=>`<li class="WorkoutCard">

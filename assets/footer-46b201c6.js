import{a as d,i as b}from"./vendor-db25513e.js";let O=document.querySelector(".QuoteText"),W=document.querySelector(".QuoteAuthor");window.addEventListener("DOMContentLoaded",async function(){try{let e=localStorage.getItem("quoteData"),t=localStorage.getItem("quoteDate"),r=new Date,s=t?new Date(t):null;if(e&&s&&U(r,s)){q(JSON.parse(e));return}let n=(await d.get("https://energyflow.b.goit.study/api/quote")).data;localStorage.setItem("quoteData",JSON.stringify(n)),localStorage.setItem("quoteDate",r.toISOString()),q(n)}catch(e){console.error("Error:",e)}});function q(e){O.innerText=e.quote,W.innerText=e.author}function U(e,t){return e.getFullYear()===t.getFullYear()&&e.getMonth()===t.getMonth()&&e.getDate()===t.getDate()}const J=document.querySelector(".FilterButtons"),f=document.querySelector(".ExerciseFiltersList"),S=document.querySelector(".Pagination"),R="https://energyflow.b.goit.study/api";let M="Muscles",v=1,k=window.innerWidth,h=0;J.addEventListener("click",z);S.addEventListener("click",w);k<=375?h=8:(k<=768,h=12);async function F(){try{return(await d.get(`${R}/filters`,{params:{filter:M,page:v,limit:h}})).data}catch(e){console.log(e)}}async function V(){try{const e=await F().then(t=>{const{results:r,totalPages:s,page:a}=t;if(r&&r.length>0){f.innerHTML=y(r);const n=L(a,s);S.innerHTML=n}else console.error("No exercises found.")})}catch(e){console.error("Error fetching exercises:",e)}}V();async function z(e){e.preventDefault(),v=1;const r=e.target.dataset.filter;if(M=r,f.innerHTML="",Array.from(e.currentTarget.children).map(s=>{s.textContent!==e.target.textContent?s.classList.remove("ButtonIsActive"):e.target.classList.add("ButtonIsActive")}),e.target.tagName==="BUTTON")try{F(r).then(s=>{const{results:a,totalPages:n,page:l}=s;f.innerHTML=y(a);{const u=L(l,n);S.innerHTML=u}})}catch(s){console.log(s)}}async function w(e){v=e.target.textContent,Array.from(e.currentTarget.children).map(t=>{t.textContent!==v?t.classList.remove("PaginationBtnIsActive"):e.target.classList.add("PaginationBtnIsActive")}),f.innerHTML="";try{const{results:t,page:r,totalPages:s}=await F();if(r===s)return;f.innerHTML=y(t)}catch(t){console.log(t)}}function y(e){return e.map(({name:r,filter:s,imgUrl:a})=>` <li class='FilterList ExercisesItem' data-filter='${s}' data-name='${r}'>
      ...
      <div class='CardHeader'>
        ...
        <div class='StartBtn' data-id='${N}'>
          ...
          <use xlink:href='/img/symbol-defs.svg#icon-arrow'></use>
        </svg>
        </div>
      </div>
      <div class='CardMainPart'>
      <div class='RunIconWrapper'><svg width='14' height='14'>
          <use xlink:href='/img/symbol-defs.svg#icon-running'></use>
        </svg></div>
